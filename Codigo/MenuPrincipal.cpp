...
void MenuPrincipal::controlesInFrame() {
	...
	if (!pulsado) { // Control una pulsación
		if (state == GLFW_PRESS) {
			//cout << "x: " << x << " ;; y: " << y << endl;
			if (x >= 0.11 && x <= 0.85 && y >= 0.57 && y <= 0.88) { // BOTÓN JUGAR
				pulsado = true;
				mode = 2;
			}
			if (x >= 0.11 && x <= 0.85 && y >= 0.21 && y <= 0.51) { // BOTÓN OPCIONES
				pulsado = true;
				mode = 3;
			}
			if (x >= 0.11 && x <= 0.85 && y >= -0.15 && y <= 0.15) { // BOTÓN PUNTUACIONES
				pulsado = true;
				mode = 4;
			}
			if (x >= 0.11 && x <= 0.85 && y >= -0.5 && y <= -0.2) { // BOTÓN CRÉDITOS;
				pulsado = true;
				mode = 5;
			}
