...
Creditos::Creditos(GLFWwindow *window) {
	...
	shaderProgram = LoadShaders("../DevilDaggers/videojuego/Codigo/Shaders/creditos.vert", "../DevilDaggers/videojuego/Codigo/Shaders/creditos.frag");

}

void Creditos::controlesInFrame() {
	...
	if (!pulsado) { // Control una pulsación
		if (state == GLFW_PRESS) {
			//cout << "x: " << x << " ;; y: " << y << endl;
			if (x >= 0.45 && x <= 0.94 && y >= -0.90 && y <= -0.75) { // BOTÓN MENÚ PRINCIPAL
				pulsado = true;
				mode = 1;
			}
		}
